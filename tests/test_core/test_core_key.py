...
class TestCoreKey(TestCase):
    ...
    def test_login_key__cleanup_error(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__no_access_key(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                  mock_handle_support_files, mock_run_script):       
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.error_result

        result = self.core.login_with_key(self.key_profile_group, None)
        self.assertEqual(self.error_result, result)

        self.assertEqual([call.cleanup()], mock_credentials.mock_calls)
        expected_key_calls = [call.check_access_key(access_key='some-access-key')]
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__fetch_session_failure(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                          mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__fetch_key_credentials_failure(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                                   mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.success_result
        mock_ensure_session.return_value = self.success_result
        mock_key.get_user_name.return_value = 'user'
        mock_key.fetch_key_credentials.return_value = self.fail_result
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__set_region_failure(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                       mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.success_result
        mock_ensure_session.return_value = self.success_result
        mock_key.get_user_name.return_value = 'user'
        mock_key.fetch_key_credentials.return_value = self.success_result
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__run_script_failure(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                       mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.success_result
        mock_ensure_session.return_value = self.success_result
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__successful_login_with_run_script_disabled(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                        mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.success_result
        mock_ensure_session.return_value = self.success_result
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__successful_login(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                     mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.success_result
        mock_ensure_session.return_value = self.success_result
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__successful_login_with_region_overwrite(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                                           mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.success_result
        mock_ensure_session.return_value = self.success_result
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__fetch_service_role_failure(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                     mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.success_result
        mock_ensure_session.return_value = self.success_result
        mock_key.get_user_name.return_value = 'user'
        mock_key.fetch_key_credentials.return_value = self.success_result
        mock_key.fetch_key_service_profile.return_value = self.fail_result
        mock_set_region.return_value = self.success_result
        mock_run_script.return_value = self.success_result
    ...
    @mock.patch('app.core.core.key')
    @mock.patch('app.core.core.credentials')
    def test_login_key__successfull_login_with_service_role(self, mock_credentials, mock_key, mock_ensure_session, mock_set_region,
                                     mock_handle_support_files, mock_run_script):
        mock_credentials.cleanup.return_value = self.success_result
        mock_key.check_access_key.return_value = self.success_result
        mock_ensure_session.return_value = self.success_result
        mock_key.get_user_name.return_value = 'user'
        mock_key.fetch_key_credentials.return_value = self.success_result
        mock_key.fetch_key_service_profile.return_value = self.success_result
        mock_set_region.return_value = self.success_result
        mock_run_script.return_value = self.success_result
